...
int main(int argc, char *argv[]) {
	...
	//penalty
	float penalty_fn, prior_beta = 0, prior_C1 = 1, prior_C2 = 1, prior_delta = 1;
	...
	if (start_iter >= 1){
		if (initial_image_filename == ""){
			printf("Image reconstruction starting from iteration %d. However, an initial image is not specified, please choose an initial image as the image for iteration %d.\n", start_iter, start_iter);
			throw 1;
		}
		else{
			printf("Image reconstruction starting from iteration %d. Using file %s as the image for iteration %d.\n", start_iter, initial_image_filename.c_str(), start_iter);
		}
	}
	...
	//
	MC_data_analysis analysis(prompts, geometry);
	analysis.write_to_file("mc_analysis.dat");
