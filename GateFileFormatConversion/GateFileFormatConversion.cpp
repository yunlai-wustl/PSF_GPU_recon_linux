...
int main(int argc, char **argv)
{
	...
	int x, y, z;
	int *decay_map;
	GATE_coincidence_event *gate_coinc_data;
	pos *pos_list;
	size_t t;
	long long int size;
	...
	if (strcmp(argv[1], "-help") == 0){
		...
		sorting_method = 1;
		printf("Using \"GATE\" file input.\n");
	}
	else{
		//use default sorting method
		sorting_method = 0;
		printf("Unrecognized sorting method, using \"mini\" instead.\n");
	}
	...
	//*****************************check file length***********************************************//
	entry_length = sizeof(GATE_coincidence_event);
	printf("Data entry length is %d\n", entry_length);
	...
	gate_coinc_data = (GATE_coincidence_event*)malloc(num_coincidence*sizeof(GATE_coincidence_event));
	current_file = fopen(file_names_ptr, "rb");
	t = fread(gate_coinc_data, sizeof(GATE_coincidence_event), num_coincidence, current_file);
	...
	for (i = 0; i<data_length; i++){
		if (gate_coinc_data[i].eventID_1 == gate_coinc_data[i].eventID_2){

			pos_list[true_coincidence] = gate_coinc_data[i].source_1;
			
			true_coincidence++;

		}
	}
	free(gate_coinc_data);
